...
static int s5k5ccaf_sensor_burst_write_list(struct samsung_short_t *list,int size, char *name)	
{
	...
	for (i = 0; i < size; i++)
	{
		...
		switch(subaddr)
		{
			...
			case 0xFFFF:
			{
				printk("s5k4ecgx_sensor_burst_write, msleep : %d\n",value);
				msleep(value);
			}
			break;
...
int sensor_af_control(char value)
{
	switch(value)
	{
		...
		s5k5ccaf_sensor_write(0x002E, 0x26FE);
		s5k5ccaf_sensor_read(0x0F12, &af_status);
		printk("[S5K5CAGX] AF_CHECK_STATUS : %d\n",  af_status);
		return af_status;
		}
		break;
...
void cam_pw(int status)
{
	...
	{
		printk("[S5K5CCAF]Camera Sensor Power ON\n");
	...
	{
		printk("[S5K5CCAF]Camera Sensor Power OFF\n");
...
static long s5k5ccaf_set_sensor_mode(int mode)
{
	...
	{
		printk("<=PCAM=> camera init for BENNET ver0.2");
	...
	const struct i2c_device_id *id)
{
	...
	printk("s5k5ccaf_probe succeeded!\n");
				...
				struct msm_sensor_ctrl *s)
{
	...
	int rc = i2c_add_driver(&s5k5ccaf_i2c_driver);
		...
		printk("[S5K5CCAF]s5k5ccaf_sensor_probe start\n");
	...
	if (rc < 0){// || s5k4ecgx_client == NULL) {
		printk("[S5K5CCAF]s5k4ecgx_sensor_probe fail\n");
		rc = -ENOTSUPP;
		goto probe_done;
	}
