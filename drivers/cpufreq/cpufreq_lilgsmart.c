/*
* drivers/cpufreq/cpufreq_lilgsmart.c
*
*
*/
...
*/
#define DEFAULT_AWAKE_IDEAL_FREQ 498200
...
*/
#define DEFAULT_DOWN_RATE_US 94000;
...
static DEFINE_PER_CPU(struct lilg_info_s, lilg_info);
...
static int cpufreq_governor_lilg(struct cpufreq_policy *policy,
unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LILGSMART
static
#endif
struct cpufreq_governor cpufreq_gov_lilgsmart = {
.name = "lilgV2",
.governor = cpufreq_governor_lilg,
.max_transition_latency = 9000000,
.owner = THIS_MODULE,
};

inline static void lilg_update_min_max(struct lilg_info_s *this_lilg, struct cpufreq_policy *policy, int suspend) {
if (suspend) {
this_lilg->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
policy->max > sleep_ideal_freq ?
(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
} else {
this_lilg->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
policy->min < awake_ideal_freq ?
(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
}
}

inline static void lilg_update_min_max_allcpus(void) {
unsigned int i;
for_each_online_cpu(i) {
struct lilg_info_s *this_lilg = &per_cpu(lilg_info, i);
if (this_lilg->enable)
lilg_update_min_max(this_lilg,this_lilg->cur_policy,suspended);
}
}
...
inline static void reset_timer(unsigned long cpu, struct lilg_info_s *this_lilg) {
this_lilg->time_in_idle = get_cpu_idle_time_us(cpu, &this_lilg->idle_exit_time);
mod_timer(&this_lilg->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct lilg_info_s *this_lilg,
int new_freq, int old_freq, int prefered_relation) {
int index, target;
struct cpufreq_frequency_table *table = this_lilg->freq_table;
...
printk(KERN_WARNING "lilg: frequency change failed: %d to %d => %d\n",
...
dprintk(lilg_DEBUG_JUMPS,"lilgQ: jumping from %d to %d => %d (%d)\n",
old_freq,new_freq,target,policy->cur);

return target;
}

static void cpufreq_lilg_timer(unsigned long cpu)
{
u64 delta_idle;
u64 delta_time;
int cpu_load;
...
u64 now_idle;
int queued_work = 0;
struct lilg_info_s *this_lilg = &per_cpu(lilg_info, cpu);
struct cpufreq_policy *policy = this_lilg->cur_policy;

now_idle = get_cpu_idle_time_us(cpu, &update_time);
old_freq = policy->cur;

if (this_lilg->idle_exit_time == 0 || update_time == this_lilg->idle_exit_time)
return;

delta_idle = cputime64_sub(now_idle, this_lilg->time_in_idle);
delta_time = cputime64_sub(update_time, this_lilg->idle_exit_time);

// If timer ran less than 1ms after short-term sample started, retry.
if (delta_time < 1000) {
if (!timer_pending(&this_lilg->timer))
reset_timer(cpu,this_lilg);
return;
}
...
dprintk(lilg_DEBUG_LOAD,"lilgT @ %d: load %d (delta_time %llu)\n",
old_freq,cpu_load,delta_time);

this_lilg->cur_cpu_load = cpu_load;
this_lilg->old_freq = old_freq;
...
(old_freq < this_lilg->ideal_speed || delta_idle == 0 ||
cputime64_sub(update_time, this_lilg->freq_change_time) >= up_rate_us))
{
dprintk(lilg_DEBUG_ALG,"lilgT @ %d ramp up: load %d (delta_idle %llu)\n",
old_freq,cpu_load,delta_idle);
this_lilg->ramp_dir = 1;
work_cpumask_set(cpu);
queue_work(up_wq, &freq_scale_work);
queued_work = 1;
}
else this_lilg->ramp_dir = 0;
}
...
(old_freq > this_lilg->ideal_speed ||
cputime64_sub(update_time, this_lilg->freq_change_time) >= down_rate_us))
{
dprintk(lilg_DEBUG_ALG,"lilgT @ %d ramp down: load %d (delta_idle %llu)\n",
old_freq,cpu_load,delta_idle);
this_lilg->ramp_dir = -1;
work_cpumask_set(cpu);
...
{
struct lilg_info_s *this_lilg = &per_cpu(lilg_info, smp_processor_id());
struct cpufreq_policy *policy = this_lilg->cur_policy;
...
if (policy->cur == policy->min && timer_pending(&this_lilg->timer))
del_timer(&this_lilg->timer);

pm_idle_old();

if (!timer_pending(&this_lilg->timer))
reset_timer(smp_processor_id(), this_lilg);
}

/* We use the same work function to sale up and down */
static void cpufreq_lilg_freq_change_time_work(struct work_struct *work)
{
unsigned int cpu;
int new_freq;
int old_freq;
int ramp_dir;
struct lilg_info_s *this_lilg;
struct cpufreq_policy *policy;
unsigned int relation = CPUFREQ_RELATION_L;
for_each_possible_cpu(cpu) {
this_lilg = &per_cpu(lilg_info, cpu);
if (!work_cpumask_test_and_clear(cpu))
continue;

ramp_dir = this_lilg->ramp_dir;
this_lilg->ramp_dir = 0;

old_freq = this_lilg->old_freq;
policy = this_lilg->cur_policy;

if (old_freq != policy->cur) {
// frequency was changed by someone else?
printk(KERN_WARNING "lilg: frequency changed by 3rd party: %d to %d\n",
old_freq,policy->cur);
new_freq = old_freq;
}
else if (ramp_dir > 0 && nr_running() > 1) {
// ramp up logic:
if (old_freq < this_lilg->ideal_speed)
new_freq = this_lilg->ideal_speed;
...
dprintk(lilg_DEBUG_ALG,"lilgQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
old_freq,ramp_dir,this_lilg->ideal_speed);
}
else if (ramp_dir < 0) {
// ramp down logic:
if (old_freq > this_lilg->ideal_speed) {
...
new_freq = old_freq * this_lilg->cur_cpu_load / max_cpu_load;
if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
new_freq = old_freq -1;
}
dprintk(lilg_DEBUG_ALG,"lilgQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
old_freq,ramp_dir,this_lilg->ideal_speed);
}
...
new_freq = old_freq;
dprintk(lilg_DEBUG_ALG,"lilgQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
old_freq,ramp_dir,nr_running());
}

// do actual ramp up (returns 0, if frequency change failed):
new_freq = target_freq(policy,this_lilg,new_freq,old_freq,relation);
if (new_freq)
this_lilg->freq_change_time_in_idle =
get_cpu_idle_time_us(cpu,&this_lilg->freq_change_time);
...
else if (timer_pending(&this_lilg->timer))
del_timer(&this_lilg->timer);
}
}
...
static struct attribute * lilg_attributes[] = {
...
static struct attribute_group lilg_attr_group = {
.attrs = lilg_attributes,
.name = "lilg",
};

static int cpufreq_governor_lilg(struct cpufreq_policy *new_policy,
unsigned int event)
{
...
this_lilg->cur_policy = new_policy;

this_lilg->enable = 1;

lilg_update_min_max(this_lilg,new_policy,suspended);

this_lilg->freq_table = cpufreq_frequency_get_table(cpu);
if (!this_lilg->freq_table)
printk(KERN_WARNING "lilg: no frequency table for cpu %d?!\n",cpu);
...
if (this_lilg->cur_policy->cur < new_policy->max && !timer_pending(&this_lilg->timer))
reset_timer(cpu,this_lilg);
...
if (this_lilg->cur_policy->cur > new_policy->max) {
dprintk(lilg_DEBUG_JUMPS,"lilgI: jumping to new max freq: %d\n",new_policy->max);
__cpufreq_driver_target(this_lilg->cur_policy,
new_policy->max, CPUFREQ_RELATION_H);
}
else if (this_lilg->cur_policy->cur < new_policy->min) {
dprintk(lilg_DEBUG_JUMPS,"lilgI: jumping to new min freq: %d\n",new_policy->min);
__cpufreq_driver_target(this_lilg->cur_policy,
...
static void lilg_suspend(int cpu, int suspend)
{
struct lilg_info_s *this_lilg = &per_cpu(lilg_info, smp_processor_id());
struct cpufreq_policy *policy = this_lilg->cur_policy;
...
lilg_update_min_max(this_lilg,policy,suspend);
if (!suspend) { // resume at max speed:
new_freq = validate_freq(policy,sleep_wakeup_freq);

dprintk(lilg_DEBUG_JUMPS,"lilgS: awaking at %d\n",new_freq);
...
dprintk(lilg_DEBUG_JUMPS,"lilgS: suspending at %d\n",policy->cur);
}

reset_timer(smp_processor_id(),this_lilg);
}

static void lilg_early_suspend(struct early_suspend *handler) {
int i;
...
static void lilg_late_resume(struct early_suspend *handler) {
int i;
...
static struct early_suspend lilg_power_suspend = {
.suspend = lilg_early_suspend,
.resume = lilg_late_resume,
#ifdef CONFIG_MACH_HERO
.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
#endif
};

static int __init cpufreq_lilg_init(void)
{
unsigned int i;
struct lilg_info_s *this_lilg;
debug_mask = 0;
...
this_lilg = &per_cpu(lilg_info, i);
this_lilg->enable = 0;
this_lilg->cur_policy = 0;
this_lilg->ramp_dir = 0;
this_lilg->time_in_idle = 0;
this_lilg->idle_exit_time = 0;
this_lilg->freq_change_time = 0;
this_lilg->freq_change_time_in_idle = 0;
this_lilg->cur_cpu_load = 0;
// intialize timer:
init_timer_deferrable(&this_lilg->timer);
this_lilg->timer.function = cpufreq_lilg_timer;
this_lilg->timer.data = i;
work_cpumask_test_and_clear(i);
}

// Scale up is high priority
up_wq = create_rt_workqueue("klilg_up");
down_wq = create_workqueue("klilg_down");
if (!up_wq || !down_wq)
return -ENOMEM;

INIT_WORK(&freq_scale_work, cpufreq_lilg_freq_change_time_work);

register_early_suspend(&lilg_power_suspend);

return cpufreq_register_governor(&cpufreq_gov_lilgsmart);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_LILGSMART
fs_initcall(cpufreq_lilg_init);
#else
module_init(cpufreq_lilg_init);
#endif

static void __exit cpufreq_lilg_exit(void)
{
cpufreq_unregister_governor(&cpufreq_gov_lilgsmart);
destroy_workqueue(up_wq);
destroy_workqueue(down_wq);
}

module_exit(cpufreq_lilg_exit);

MODULE_AUTHOR ("lilg");
MODULE_DESCRIPTION ("'cpufreq_lilgsmart' - A lilg cpufreq governor");
MODULE_LICENSE ("GPL");
